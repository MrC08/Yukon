			...
			if c == "action:wait":
				...
				if len(drinks) == 0:
					...
					if c != 0:
						...
						doWhile = True
						while doWhile:
							inventory[drinksInternal[c]] -= 1
							food += Item(drinksInternal[c]).getFood()
							water += Item(drinksInternal[c]).getWater()

							doWhile = inventory[drinksInternal[c]] > 0 and water < (100 - Item(drinksInternal[c]).getWater())
